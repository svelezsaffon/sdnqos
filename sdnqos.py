...
class RestQoSAPI(app_manager.RyuApp):
    ...
    def santiago_QoS(self, ev):
		print "We have controlnow"
		
		print "The Switch ID is="
		dpid=ev.dp.id
		print dpid
		
		if dpid in QoSController._OFS_LIST:
			print "We have this Switch :)"
			print "Here is the QoS manager"
			...
			print "Correct ovs Address"
			#def set_queue(self, rest, vlan_id):
			#cal the QoS to create the rest parameters			
			command=ofs.set_queue(rest_cmd,VLANID_NONE)
			print "Results :"
			
			
			
			if command['command_result']['result']=='failure':
				print command['command_result']['result']
				print command['command_result']['details']
			else:
				print "Got answer form switch"
				print command['command_result']['details']
				
				
			##def set_queue(self, req, switchid, **_kwargs):			
			##QoSController.set_queue(,dpid,
			
		else:
			print "We don't have this Switch :)"
    ...
    def handler_datapath(self, ev):
        if ev.enter:
            ...
            print "New Cool Controller"
            self.santiago_QoS(ev)            
        else:
...
class QoSController(ControllerBase):
    ...
    def regist_ofs(dp, CONF):
        ...
        try:
            ...
            print "We also have a QoS Manager"
